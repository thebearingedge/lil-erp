...
create function create_purchase_order(stream_id uuid, payload jsonb) returns void as $$
  declare
...
create function event_create_purchase_order() returns trigger as $$
  begin
    perform create_purchase_order(new.stream_id, new.payload);
    return new;
  end;
$$ language plpgsql;
...
create trigger create_purchase_order
  ...
  on event_store
  for each row
  when (new.event_type = 'create_purchase_order')
...
drop function create_purchase_order(stream_id uuid, payload jsonb);
